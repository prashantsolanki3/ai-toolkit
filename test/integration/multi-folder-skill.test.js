...
test('claude-code (dir destination): multi-folder skill copies the entire tree', async () => {
  ...
  try {
    ...
    for (const rel of EXPECTED_FILES) {
      assert.ok(
        fs.existsSync(path.join(dir, 'skills', MULTI_FOLDER_SKILL, rel)),
...
test('antigravity (dir destination, root-level skills): multi-folder skill round-trips', async () => {
  ...
  try {
    ...
    for (const rel of EXPECTED_FILES) {
      assert.ok(fs.existsSync(path.join(dir, MULTI_FOLDER_SKILL, rel)));
    }
  } finally {
    cleanupTmpProject(target);
  }
...
test('cursor (file destination): multi-folder skill flattens to a single .mdc', async () => {
  ...
  try {
    ...
    assert.ok(
      fs.existsSync(path.join(dir, 'rules', `${MULTI_FOLDER_SKILL}.mdc`)),
      `missing flattened .mdc`,
    );
    assert.ok(!fs.existsSync(path.join(dir, 'rules', 'scripts')));
    assert.ok(!fs.existsSync(path.join(dir, 'rules', 'references')));
  } finally {
    cleanupTmpProject(target);
  }
});
...
test('update: changes to nested files in a multi-folder skill propagate', async () => {
  ...
  try {
    ...
    const installedNested = path.join(
      toolDir(target, 'claude-code'),
      'skills',
      MULTI_FOLDER_SKILL,
      'references',
      'style-guide.md',
...
test('remove: tears down the entire multi-folder skill directory', async () => {
  ...
  try {
    ...
    assert.equal(
      fs.existsSync(path.join(toolDir(target, 'claude-code'), 'skills', MULTI_FOLDER_SKILL)),
      false,
    );
  } finally {
    cleanupTmpProject(target);
