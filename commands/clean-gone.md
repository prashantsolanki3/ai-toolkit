...
5. **Gentle removal.** For each surviving candidate:
   - Find the worktree path: `wt=$(git worktree list --porcelain | awk -v b="$branch" '/^worktree / { wt = substr($0, 10) } $1 == "branch" && $2 == "refs/heads/" b { print wt; exit }')`. `git worktree list --porcelain` emits records of `worktree <path>` / `HEAD <sha>` / `branch refs/heads/<name>` (blank-line separated), so we track the worktree path as we scan and emit it when the branch matches. `substr($0, 10)` skips the literal `"worktree "` prefix (9 chars + trailing space) and keeps the full path including spaces — `$2` alone would truncate `/path/with spaces/wt` at the first space. The earlier `{print prev}` shortcut was wrong twice: the line right before `branch` is `HEAD <sha>`, not the worktree path, and field-splitting truncates paths with spaces.
