...
echo "[1/8] install --dry-run plans without writing (--target defaults to CWD)"
cd "$SCRATCH"
...
echo ""
echo "[2/8] install --preset backend-essentials --tool claude-code (lands in .claude/)"
...
echo ""
echo "[3/8] installed shows what's there (no --tool — autodiscovers)"
...
echo ""
echo "[8/8] remove --all (--tool cursor) clears every tracked asset and updates the lockfile"
